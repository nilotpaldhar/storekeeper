...
// Query: *[_type in ["taxonomy", "taxon"]]{ "slug": slug.current }[0...$limit]
export type CategorySlugsQueryResult = Array<{
	slug: string | null;
}>;
// Variable: CategoryQuery
// Query: *[_type in ["taxonomy", "taxon"] && slug.current ==  $slug] | order(_updatedAt desc) [0] {        "type": _type,        "id": _id,        title,        "slug": slug.current,    }
...
// Variable: SyncPublishedProductsQuery
// Query: *[ _type == "product" && !(_id in path('drafts.**'))] {		    "id": _id,    title,    "slug": slug.current,    description,    hasVariants,    "sku": sku->{     "id": _id,    code,    name,    description,    imageUrl,    piecesPerPack,    weight,    unitOfWeight,    hsTariffNumber },    "taxonomy":  taxon->taxonomy->{      "id": _id,    title,    "slug": slug.current },    "taxon": taxon->{     "id": _id,    title,    "slug": slug.current,    isLeaf },    "brand": brand->{     "id": _id,    title,    "slug": slug.current },    "gallery":  select(        defined(gallery) => gallery[]{     "refKey": _key,    "image": image.asset._ref,    altText },        []    ),    "variants": select(        defined(variants) => variants[]{     "refKey": _key,    variantKey,    "sku": sku->{     "id": _id,    code,    name,    description,    imageUrl,    piecesPerPack,    weight,    unitOfWeight,    hsTariffNumber },    "gallery":  select(        defined(gallery) => gallery[]{     "refKey": _key,    "image": image.asset._ref,    altText },        []    ) },        []    ),	}
export type SyncPublishedProductsQueryResult = Array<{
	id: string;
	title: string | null;
	slug: string | null;
	description: string | null;
...
// Variable: CategorySeo
// Query: *[_type in ["taxonomy", "taxon"] && slug.current ==  $slug] | order(_updatedAt desc) [0] {        seo {     metaTitle,    metaDesc,    shareTitle,    shareDesc,    "shareGraphic": shareGraphic.asset->url }    }
export type CategorySeoResult = {
	seo: {
		metaTitle: string | null;
		metaDesc: string | null;
		shareTitle: string | null;
...
// Variable: TaxonomyBySlug
// Query: *[_type == "taxonomy" && slug.current == $slug][0]{		"id": _id,		title,		"slug": slug.current	}
export type TaxonomyBySlugResult = {
	id: string;
	title: string | null;
	slug: string | null;
} | null;
...
declare module "@sanity/client" {
	interface SanityQueries {
		...
		'\n\t*[ _type == "product" && !(_id in path(\'drafts.**\'))] {\n\t\t\n    "id": _id,\n    title,\n    "slug": slug.current,\n    description,\n    hasVariants,\n    "sku": sku->{ \n    "id": _id,\n    code,\n    name,\n    description,\n    imageUrl,\n    piecesPerPack,\n    weight,\n    unitOfWeight,\n    hsTariffNumber\n },\n    "taxonomy":  taxon->taxonomy->{  \n    "id": _id,\n    title,\n    "slug": slug.current\n },\n    "taxon": taxon->{ \n    "id": _id,\n    title,\n    "slug": slug.current,\n    isLeaf\n },\n    "brand": brand->{ \n    "id": _id,\n    title,\n    "slug": slug.current\n },\n    "gallery":  select(\n        defined(gallery) => gallery[]{ \n    "refKey": _key,\n    "image": image.asset._ref,\n    altText\n },\n        []\n    ),\n    "variants": select(\n        defined(variants) => variants[]{ \n    "refKey": _key,\n    variantKey,\n    "sku": sku->{ \n    "id": _id,\n    code,\n    name,\n    description,\n    imageUrl,\n    piecesPerPack,\n    weight,\n    unitOfWeight,\n    hsTariffNumber\n },\n    "gallery":  select(\n        defined(gallery) => gallery[]{ \n    "refKey": _key,\n    "image": image.asset._ref,\n    altText\n },\n        []\n    )\n },\n        []\n    ),\n\n\t}\n': SyncPublishedProductsQueryResult;
